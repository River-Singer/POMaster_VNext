# -*- coding: utf-8 -*-
"""render_registry_tree.py —— D25 registry-tree 投影渲染器 v0（原型）。

把 vNext truth 对象（corpus/master/batch-*/truth/objects/**）按批次过滤后
反向组装成旧 PoMaster 时代 outputs/frontend/10_planned 的 registry YAML 形状
（扩展名 .yaml、内容为 JSON，键序固定，与旧文件同一序列化纪律）。

架构声明（catalog/projection-presets/registry-tree.yaml §preset.renderer）：
...
seq 锚：MIG-AUTH-0001（语料批代号 MIG-AUTH；采集语境无全局 seq 分配器，
本常量只作工作单元锚记，不伪造事件序号）。

用法：
  python tools/render_registry_tree.py --batch-dir ../batch-1 --out ../renders/batch-1
  python tools/render_registry_tree.py --batch-dir ../batch-1 ../batch-2 \
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="registry-tree projection renderer v0")
    parser.add_argument("--batch-dir", nargs="+", required=True,
                        help="corpus/master/batch-N dirs containing truth/objects")
    parser.add_argument("--preset", default=os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__))))),
        "catalog", "projection-presets", "registry-tree.yaml"))
