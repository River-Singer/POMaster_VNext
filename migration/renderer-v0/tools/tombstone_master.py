...
授权：POMaster_VNext/migration/AUTHORIZATION.md（Owner 2026-08-29「授权走迁移线」）。
...
VNEXT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__)))))  # migration/renderer-v0/tools -> POMaster_VNext
INVENTORIES = [
    os.path.join(VNEXT_ROOT, "migration", "master-batch1", "inventory.yaml"),
    os.path.join(VNEXT_ROOT, "migration", "master-batch2", "inventory.yaml"),
]
...
#   succumbed_to_batch : {batch}（POMaster_VNext/migration/{batch_dir}）
#   migrated_to        : {migrated_to}
#   收编名册/盘点 pin  : POMaster_VNext/migration/{batch_dir}/inventory.yaml
# DO NOT EDIT —— 自冻结起禁止任何修改（人手与治理工具自动写入均禁止）：
#   变更事实的唯一入口是 vNext Canonical State（Transition + Authority）；
#   本文件此后仅是旧布局（projection preset: legacy-outputs）的冻结历史件。
#   对本文件的写入会被 .git/hooks/pre-commit 冻结守卫拒绝（卸载方法见该文件头：
#   rm .git/hooks/pre-commit）。
# 施工：MASTer_master 分支 migration/mig-b1-b2-tombstone ·
#   写授权记录：POMaster_VNext/migration/AUTHORIZATION.md（Owner 2026-08-29）
# ============================================================================
"""
...
def migrated_to(batch_dir):
    if batch_dir == "master-batch2":
        return ("POMaster_VNext/migration/master-batch2/truth/objects/**"
                "（component-selection-register 为 inventory 在册零对象，显式缺席登记）")
    return "POMaster_VNext/migration/%s/truth/objects/**" % batch_dir
