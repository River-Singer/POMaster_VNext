 ...
 * 文件后重跑仍零变化）、B7-THEME 清单现状 pin（缺省装载 103 份播种件 = themes 21 +
 * stacks 36 + baseline 26 + evidence 20，B6C + B6D + B6E + B6F + B6G + B7-THEME 六批合并清单；
 * stacks slug 三面单源
...
// vendor stacks 分母侧目录（旧包 pomaster/ 平级只读依赖；B6c 后端族 = backend-hard-
// spec、B6f 前端族 = frontend-hard-spec；fresh clone/CI 无此兄弟目录 → vendor 侧断言
// 显式 skip，manifest 侧单源断言不受影响——宿主缺席诚实 not_run 先例）。
const VENDOR_REPO_COMPONENTS = join(
  fileURLToPath(new URL("../../../", import.meta.url)),
  "..",
  "pomaster",
  "components",
...
const VENDOR_STACKS_DIR = join(
  VENDOR_REPO_COMPONENTS,
  "backend-hard-spec",
  "assets",
  "stacks",
);
const VENDOR_FSTACKS_DIR = join(
  VENDOR_REPO_COMPONENTS,
  "frontend-hard-spec",
  "assets",
  "stacks",
);
const VENDOR_STACKS_PRESENT =
  existsSync(VENDOR_STACKS_DIR) && existsSync(VENDOR_FSTACKS_DIR);
...
describe("seedProjectAssets 目录守卫（R4 红线 + B6b-I 播种 allowlist 收窄）", () => {
  ...
  it.skipIf(!VENDOR_STACKS_PRESENT)("B6c stacks slug 集 == 种子清单 stacks 分母派生集合（三面单源对账——slug 漂移即爆；vendor 平级目录缺席时宿主诚实 skip）", () => {
    ...
    // vendor stacks 分母侧（播种源目录——后端族 B6c 14 slug + 前端族 B6f 3 slug +
    // B6G css，profiles/ 除外，恰 18 个）。
    const vendorSlugs = (dir: string) =>
      readdirSync(dir, { withFileTypes: true })
        .filter((d) => d.isDirectory() && d.name !== "profiles")
        .map((d) => d.name);
    const slugs = [...vendorSlugs(VENDOR_STACKS_DIR), ...vendorSlugs(VENDOR_FSTACKS_DIR)].sort();
...
describe("runInit 步骤 4.6 播种端到端（fresh 临时工程 + 注入清单）", () => {
  ...
  it("B7-THEME 清单现状 pin：缺省装载 103 份播种件（themes 21 + stacks 36 + baseline 26 + evidence 20；六批合并全量分母——R3 tokens 补位 102→103）——目标全落播种 allowlist 面且分面形态正确", () => {
    const entries = loadSeedManifestEntries();
    ...
    for (const entry of entries) {
      ...
      const inEvidence = /^\.pomaster\/specs\/evidence\/[a-z0-9-]+\.md$/.test(entry.path);
      expect(inSpecs || inBaseline || inEvidence, entry.path).toBe(true);
      if (inSpecs) {
        ...
        // evidence 面 B6e 新著件：纯正文（authoring=new 通路复用）；spec 件带十七段
        // 结构锚词形，index 件带登记级衔接注记（结构面在 evidence-seeds.spec 全量钉）。
        expect(entry.content.startsWith("---\n"), entry.path).toBe(false);
        if (entry.path.endsWith("/index.md")) {
          expect(entry.content.includes("对象面衔接"), entry.path).toBe(true);
        } else {
          expect(entry.content.includes("## Assertions"), entry.path).toBe(true);
          expect(entry.content.includes("## Purpose"), entry.path).toBe(true);
          expect(entry.content.includes("SPEC."), entry.path).toBe(true);
        }
      } else {
    ...
    // baseline：分区计数 1+8+8+5+4 = 26（B6d 分母钉 + R3 tokens 补位；逐文件集合钉在 baseline-seeds.spec）。
  ...
  it("缺省（不注入）init：103 份播种件在位（seeded 报告 + 幂等重跑全 preserved）", async () => {
    ...
    expect(existsSync(join(dir, ".pomaster", "specs", "hard", "stacks", "vue3", "index.md"))).toBe(true);
    expect(existsSync(join(dir, ".pomaster", "specs", "hard", "stacks", "vue3", "vue3-framework-overlay.md"))).toBe(true);
    // B6G css 体系抽查：css index/overlay 落位。
    expect(existsSync(join(dir, ".pomaster", "specs", "hard", "stacks", "css", "index.md"))).toBe(true);
    expect(existsSync(join(dir, ".pomaster", "specs", "hard", "stacks", "css", "css-system-overlay.md"))).toBe(true);
    ...
    // evidence 面（B6e）：index + 19 spec 全落（十七段结构锚词形抽查；README.md 是
    // init 布局预铺物非播种件，不计——npm 冒烟同款口径）。
    const evidenceDir = join(dir, ".pomaster", "specs", "evidence");
    const evidenceFiles = existsSync(evidenceDir)
      ? readdirSync(evidenceDir).filter((name) => name.endsWith(".md") && name !== "README.md").sort()
      : [];
    expect(evidenceFiles).toHaveLength(20);
    expect(evidenceFiles[0]).toBe("accessibility.md");
    expect(evidenceFiles.includes("index.md")).toBe(true);
    expect(evidenceFiles.includes("complexity-crap.md")).toBe(true);
    expect(evidenceFiles[19]).toBe("visual-regression.md");
    const crapSeed = readFileSync(join(evidenceDir, "complexity-crap.md"), "utf8");
    expect(crapSeed.includes("## Purpose") && crapSeed.includes("### NOT_RUN")).toBe(true);
    expect(crapSeed.startsWith("---\n")).toBe(false);
    // 幂等：重跑全 preserved = NO_CHANGE。
    const second = await runInit(dir);
    expect(second.ok).toBe(true);
    expect(second.result.change).toBe("NO_CHANGE");
    expect(second.result.files.filter((f) => f.action === "preserved")).toHaveLength(103);
  });
