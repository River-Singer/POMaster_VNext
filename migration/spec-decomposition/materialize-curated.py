...
HERE = os.path.dirname(os.path.abspath(__file__))                      # .../migration/spec-decomposition
...
CLEAN_ROOM_NOTE = ("independently rewritten from SPEC-D decomposition candidate cards; "
                   "zero verbatim copy")
POOL_REL = "POMaster_VNext/migration/spec-decomposition/candidates/consolidated-pool.yaml"
...
def build_entry(rank, pool_rec, card, statement):
    ...
    entry = {
        ...
        "sources": [
            {
                "type": "design_seed",
                "ref": "POMaster_VNext/migration/spec-decomposition/candidates/%s.yaml" % group,
...
def merge_lock(old_lock, new_metas):
    ...
    lock = dict(old_lock)
    lock["generated_by"] = (
        "catalog/tools/materialize_batch4_uplift.py（MIG-B4 Universal 上提；entries 按 id 排序；"
        "在 materialize_catalog_pilot.py 60 条基础上追加 9 条）+ "
        "migration/spec-decomposition/materialize-curated.py（SPEC-D 汇总池 D5 精选追加 25 条）"
    )
...
def build_backlog_doc(rest):
    return {
        "meta": {
            ...
            "built_by": "migration/spec-decomposition/materialize-curated.py",
            "inputs": [POOL_REL],
...
def build_all():
    ...
    for rank, pool_rec in enumerate(curated, 1):
        ...
        metas.append({
            ...
            "source_ref": "POMaster_VNext/migration/spec-decomposition/candidates/%s.yaml" % card["group"],
        })
...
def run():
    ...
    if write_if_changed(backlog_path, b1["backlog_bytes"]):
        written.append("migration/spec-decomposition/backlog-registered.yaml")
