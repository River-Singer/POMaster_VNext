...
describe("v0.6.1 §91 P0 Acceptance（八条）", () => {
  ...
  it("⑦ Catalog 人类可完整浏览：catalog status 分母含 archetypes=41 + explain 单条目解释", async () => {
    const status = await runCatalogStatus({ catalogRoot });
    expect(status.ok).toBe(true);
    expect(status.result?.sections.archetypes).toBe(41);
  ...
  it("⑧ Agent Context 只注入任务所需标准件：matches ⊂ 分母且 required_bindings 仅结构面", async () => {
    ...
    expect(outcome.sources_examined.catalog_archetypes).toBe(41);
    expect(outcome.matches.length).toBeLessThan(41);
