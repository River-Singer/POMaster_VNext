...
truth objects for the mechanically nameable entries, one per rule entry:
    ...
    -> migration/master-batch3/truth/objects/business-rule/<page_seg>/<local>.json
...
def build_envelope(
    ...
    escalation = (
        "regenerate via migration/master-batch3/tools/ingest_business_rule_registry.py; "
...
def build_pending_manifest(rules, gated, source_digest, transcribed_count, census):
    ...
    manifest = {
        "adjudications": [
            {
                "adjudicated_by": "migration/master-batch3/tools/ingest_business_rule_registry.py (M2 group 3)",
        ...
        "generated_by": "migration/master-batch3/tools/ingest_business_rule_registry.py",
        "grants": [
            {
