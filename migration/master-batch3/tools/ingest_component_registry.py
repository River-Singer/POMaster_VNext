...
truth objects -- the NON-GRID remainder of components[]:
    ...
    (kind=capability) -> migration/master-batch3/truth/objects/capability/
...
CAPABILITY.GRID.COLUMN_CONFIG / CAPABILITY.GRID.EDITABLE_GRID (migration/
master-batch1/truth/objects/capability/grid.*.json). This tool reads that
...
def check_ledger(ledger):
    for entry in ledger.get("entries", []):
        if entry.get("kind") == "component-registry":
            ...
            if dest != "migration/master-batch3/truth/objects/capability/":
...
def build_envelope(src, entry, entry_index, total, source_digest, corr):
    ...
    escalation = (
        "regenerate via migration/master-batch3/tools/ingest_component_registry.py; "
