...
Read-only over MASTer_master/.trellis/spec. Writes exactly one file:
  migration/spec-decomposition/spec-inventory.yaml
...
# REPO_ROOT = po-master (tools/ -> spec-decomposition/ -> migration/ -> POMaster_VNext/ -> po-master)
POMASTER_VNEXT = os.path.join(REPO_ROOT, "POMaster_VNext")
OUT_DIR = os.path.join(POMASTER_VNEXT, "migration", "spec-decomposition")
...
def build_document():
    ...
    doc = {
        "meta": {
            ...
            "discipline": [
                "MASTer_master 只读：本工具只读 .trellis/spec 全树，产出仅写 migration/spec-decomposition/",
...
def main() -> int:
    ...
    print("written: migration/spec-decomposition/spec-inventory.yaml (%d bytes)" % len(data))
