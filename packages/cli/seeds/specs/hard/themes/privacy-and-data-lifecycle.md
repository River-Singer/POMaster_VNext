...
related_tools: []
legacy_id: theme:privacy-and-data-lifecycle
...
triggers: [sensitive-data, privacy, data-export] # 聚合注记：来源 triggers 并集；info 性注记非执行语义
...
x-aggregation: # 聚合来源（D6）：逐源 vendor pin（sha256 与卡 vendor_pin 同值）
  - seed_source: pomaster/components/frontend-hard-spec/assets/universal/44-privacy-data-lifecycle-protocol.md
    sha256: 5b3472bba296c1bb437c3ce41e1c937b636605e39b78cf06747d1010e46ba08e
    seed_version: B6B-2
  - seed_source: pomaster/components/backend-hard-spec/assets/universal/13-privacy-data-lifecycle-protocol.md
    sha256: df20ccb85008f8744ea16db3ea64e54bdf9fd19d7aa3098c45dc44fff94c3d8d
    seed_version: B6C
---
...
**源：FE 44 隐私与客户端数据生命周期协议（pomaster/components/frontend-hard-spec/assets/universal/44-privacy-data-lifecycle-protocol.md）**

P0。约束个人、敏感和可关联数据在前端的收集、访问、传输、存储、复制、第三方共享、保留、撤回与清理。

**源：BE 13 隐私与数据生命周期协议（pomaster/components/backend-hard-spec/assets/universal/13-privacy-data-lifecycle-protocol.md）**

规范数据最小化、分类、脱敏、保留、删除、导出、接收方与审计。

## Non-Scope

**源：FE 44 隐私与客户端数据生命周期协议**
...
**源：BE 13 隐私与数据生命周期协议 · §MUST**

- 敏感数据必须有目的、来源、接收方、保留期、删除触发与访问控制。

## MUST NOT

**源：FE 44 隐私与客户端数据生命周期协议 · §MUST NOT**
...
**源：BE 13 隐私与数据生命周期协议 · §Contract**

数据设计必须记录分类、处理目的、存储位置、流向、保留与删除证据。

## Checklist

**源：FE 44 隐私与客户端数据生命周期协议 · §Checklist**
...
**源：BE 13 隐私与数据生命周期协议 · §Change Policy**
