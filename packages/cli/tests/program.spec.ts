...
describe("runCli --json 机读契约（§45）", () => {
  ...
  it("doctor --json → 矩阵含 kernel / 三工具探针（P22）+ coverage 双腿（P23）+ mutation 双腿（P24）+ security 三腿（P25）+ playwright（P26）+ performance 双 runner 与 schemathesis（P27）/ chrome_devtools_mcp + playwright_mcp（批次 2.6 Browser Eyes 双 MCP 各自显式）/ heavy_entry_hooks + heavy_entry_skills（重入口安装物）/ portability_runtime_rebuild（P32）/ sensor_capability_catalog（P1-5 裁决 8-D7）二十一探针", async () => {
    const io = capture();
    ...
    // 重入口探针：未 init 目录 → MISSING_CONFIGURATION（应装未装显式，init 路标）。
    const hooks = probes.find((p) => p.probe === "heavy_entry_hooks");
