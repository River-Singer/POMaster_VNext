 ...
 * 3) 收敛：新增物料 → added 且 entries/allowed/required 三方 228→229 对账绿；
...
describe("catalog relock（漂移恢复键：漂移→relock→回绿）", () => {
  it("手改 archetype 不重锁 → status exit 1（CATALOG_LOCK_DRIFT）→ relock → status 回绿 exit 0（added=0/refreshed=1）", async () => {
    ...
    expect(relocked.result.entries_total).toBe(228);
    ...
    expect(human).toContain("catalog-lock: relocked & verified（228 entries）");
    expect(human).toContain(`~ ${TAMPER_PATH}`);

    const green = await runCatalogStatus({ catalogRoot });
    expect(green.ok).toBe(true);
    expect(green.result.lock_verification).toEqual({ ok: true, entries_checked: 228, drifts: [] });
  ...
  it("controlled_children 内扩展键保留（vNext Batch 4 R3 教训修复：note 是管辖面语义注记，重建 allowed/required 不得丢弃扩展键）", async () => {
    ...
    expect(after.controlled_children["allowed"]).toHaveLength(228);
    expect(after.controlled_children["required"]).toHaveLength(228);
  });

  it("收敛：新增物料 → added 含该路径且 entries/allowed/required 三方 228→229 对账绿；删除 → removed 且对账绿", async () => {
    const catalogRoot = trackTempCatalog();
    ...
    expect(addedRun.result.entries_total).toBe(229);
    const lock = readCatalogLock(catalogRoot);
    expect(lock.entries).toHaveLength(229);
    expect(lock.controlled_children.allowed).toHaveLength(229);
    expect(lock.controlled_children.required).toHaveLength(229);
    ...
    expect(greenStatus.result.entries_total).toBe(229);
    ...
    expect(removedRun.result.entries_total).toBe(228);
    expect(readCatalogLock(catalogRoot).entries).toHaveLength(228);
    expect(verifyCatalogLock(catalogRoot).ok).toBe(true);
  });
  ...
  it("store 零依赖：无 .pomaster 的项目根同样可跑（catalog 是工具侧资产 §92.2；runCli exit 0 + §45 信封）", async () => {
    ...
    expect(envelope.result["entries_total"]).toBe(228);
