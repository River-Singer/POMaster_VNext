...
describe("readCatalogLock（lock 文档形态）", () => {
  it("版本/profile/entries 分母与排序（分母锁：253 entries，id 确定性排序；P-v06 批次 2 增量 111→123、批次 2.6 Browser Eyes 123→124、批次 3 Backend/API/Data 124→141、批次 4 RUNTIME 物料 141→143、B6b-I FE 移植 143→168、B6b-II FE 移植 168→193、B6c BE+stacks 移植 193→228、裁定批 F/D3-R1 复核批 228→253）", () => {
    ...
    expect(lock.entries.length).toBe(253);
    expect(lock.controlled_children.allowed.length).toBe(253);
    expect(lock.controlled_children.required.length).toBe(253);
...
describe("loadCatalogPolicies（policies 物料读取）", () => {
  it("分母锁：189 条（authority.* 与 policy.* 同为 kind=policy；B6b-I FE 移植 +25：79→104；B6b-II FE 移植 +25：104→129；B6c BE+stacks 移植 +35：129→164——policy 面 25 + TECHNOLOGY_PROFILE 面 10；裁定批 F/D3-R1 逐卡复核 +25：164→189）", () => {
    expect(loadCatalogPolicies(REPO_CATALOG).length).toBe(189);
  });
  ...
  it("lane 分布对账（any 120 / frontend 44 / backend 25；与 vocab-lock V7 词形一致；B6b-I 25 条 lane=any：43→68；B6b-II 25 条 any 17 / frontend 8：68→85、36→44；B6c 35 条 any 24 / backend 11：85→109、0→11；D3-R1 25 条 any 11 / backend 14：109→120、11→25）", () => {
    const policies = loadCatalogPolicies(REPO_CATALOG);
    expect(policies.filter((p) => p.lane === "any").length).toBe(120);
    expect(policies.filter((p) => p.lane === "frontend").length).toBe(44);
    expect(policies.filter((p) => p.lane === "backend").length).toBe(25);
  });
...
describe("verifyCatalogLock（repo 实物：producer 与对账端同口径）", () => {
  it("全量对账 ok：253 entries 哈希 + 管辖面双向对账零漂移（P-v06 批次 2 增量 111→123、批次 2.6 Browser Eyes 123→124、批次 3 Backend/API/Data 124→141、批次 4 RUNTIME 物料 141→143、B6b-I FE 移植 143→168、B6b-II FE 移植 168→193、B6c BE+stacks 移植 193→228、裁定批 F/D3-R1 复核批 228→253）", () => {
    const verification = verifyCatalogLock(REPO_CATALOG);
    expect(verification).toEqual({ ok: true, entries_checked: 253, drifts: [] });
  });
});
...
describe("relockCatalog（重锁计算：纯计算零写盘，Owner 裁决 2026-09-03）", () => {
  it("漂移重算：next.entries 哈希 = 落盘实际字节；refreshed 精确指路；previous 与磁盘 lock 字节零触碰", () => {
    ...
    expect(report.next.entries).toHaveLength(253);
    expect(report.previous.entries).toHaveLength(253);
  ...
  it("收敛：新增物料进 next（added + source_ref 确定性缺省）；删除物料出 next（removed）", () => {
    ...
    expect(added.next.entries).toHaveLength(254);
    expect(added.next.controlled_children.allowed).toHaveLength(254);
    expect(added.next.controlled_children.required).toHaveLength(254);
    ...
    expect(removed.next.entries).toHaveLength(253);
    expect(removed.next.controlled_children.allowed).toHaveLength(253);
    expect(removed.next.controlled_children.required).toHaveLength(253);
  });
