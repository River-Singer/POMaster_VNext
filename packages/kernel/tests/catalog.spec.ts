 ...
 *    sha256(utf-8 字节) 与对账端同源；entries 分母 270（policies 206/gates 6/knowledge 11/sensors 6/archetypes 41——P1-5 sensors 六条目 + P-v06 批次 1 archetypes 十条目与 GATE.NEW_ENTITY.CHECKS 登记 + P-v06 批次 2 archetypes 十二条目 + P-v06 批次 2.6 Browser Eyes：knowledge 10→11（KNOWLEDGE.WEB.BROWSER.MCP_EYES）+ P-v06 批次 3 archetypes 十七条目（Backend/API/Data，backend-references.md 2026-09-03 锚）+ P-v06 批次 4 archetypes 两条目（RUNTIME：ENVIRONMENT_PARITY/OBSERVABILITY_BINDING，runtime-references.md 2026-09-03 锚）+ B6b-I FE 播种移植物料 25 条（policies 79→104：catalog/tools/seed_b6b_frontend.py——D5 精选 22 required + 3 advisory，x-b6-porting 注记 + enforcement 轴断言）+ B6b-II FE 播种移植物料 25 条（policies 104→129：同工具 B6B-2 批——FE 24-45+index 分母，D5 上限 25/批内执行）+ B6c BE+stacks 播种移植物料 35 条（policies 129→164：catalog/tools/seed_b6c_backend.py——BE universal 分母 policy 面 25 [22 required + 3 advisory] + stacks/profile TECHNOLOGY_PROFILE 分类面 10）+ 裁定批 F/D3-R1 逐卡复核物料 25 条（policies 164→189：catalog/tools/seed_d3_review.py——72 张保守排除卡复核：required 9 补锚 + advisory 16，维持排除 30 + 留 D3-R2 17）+ 裁定批 G/D3-R2 收尾轮物料 17 条（policies 189→206：同工具 D3-R2 批——R1 后余量 advisory 全量入册，13 SHOULD 锚 + 4 扩展段/Change Policy 锚，新 id 域段 BE 候选注记在册））。
...
describe("readCatalogLock（lock 文档形态）", () => {
  it("版本/profile/entries 分母与排序（分母锁：270 entries，id 确定性排序；P-v06 批次 2 增量 111→123、批次 2.6 Browser Eyes 123→124、批次 3 Backend/API/Data 124→141、批次 4 RUNTIME 物料 141→143、B6b-I FE 移植 143→168、B6b-II FE 移植 168→193、B6c BE+stacks 移植 193→228、裁定批 F/D3-R1 复核批 228→253、裁定批 G/D3-R2 收尾轮 253→270）", () => {
    ...
    expect(lock.entries.length).toBe(270);
    expect(lock.controlled_children.allowed.length).toBe(270);
    expect(lock.controlled_children.required.length).toBe(270);
...
describe("loadCatalogPolicies（policies 物料读取）", () => {
  it("分母锁：206 条（authority.* 与 policy.* 同为 kind=policy；B6b-I FE 移植 +25：79→104；B6b-II FE 移植 +25：104→129；B6c BE+stacks 移植 +35：129→164——policy 面 25 + TECHNOLOGY_PROFILE 面 10；裁定批 F/D3-R1 逐卡复核 +25：164→189；裁定批 G/D3-R2 收尾轮 +17：189→206）", () => {
    expect(loadCatalogPolicies(REPO_CATALOG).length).toBe(206);
  });
  ...
  it("lane 分布对账（any 130 / frontend 44 / backend 32；与 vocab-lock V7 词形一致；B6b-I 25 条 lane=any：43→68；B6b-II 25 条 any 17 / frontend 8：68→85、36→44；B6c 35 条 any 24 / backend 11：85→109、0→11；D3-R1 25 条 any 11 / backend 14：109→120、11→25；D3-R2 17 条 any 10 / backend 7：120→130、25→32）", () => {
    const policies = loadCatalogPolicies(REPO_CATALOG);
    expect(policies.filter((p) => p.lane === "any").length).toBe(130);
    expect(policies.filter((p) => p.lane === "frontend").length).toBe(44);
    expect(policies.filter((p) => p.lane === "backend").length).toBe(32);
  });
...
describe("loadCatalogTools / loadCatalogProjectionPresets", () => {
  it("tools 消费：13 份实存工具（懒加载清单分母；P-v06 批次 2 增量 5→6、批次 3 增量 6→7、批次 4 增量 7→8、B6b-I 移植工具 8→9、B6c BE 移植工具 9→10、B6d baseline 新著工具 10→11、B6e evidence 新著工具 11→12、裁定批 F/D3 复核工具 12→13）", () => {
  ...
  it("tools 消费滤 `__pycache__`：Python import 缓存目录不进分母（D8-1 / 裁决 12；B6b/B6c check 各撞一次的弱点封堵）", () => {
    const catalogRoot = trackTempCatalog();
    // recursive 幂等：fixture cpSync 会连带复制工作树的实存 __pycache__（Python
    // import 缓存即红的正是本测试要封堵的环境态）——非递归 mkdir 在此 EEXIST 即红。
    mkdirSync(join(catalogRoot, "tools", "__pycache__"), { recursive: true });
    writeFileSync(
    ...
    expect(tools).toHaveLength(13); // 13 份实存 .py 分母不被缓存目录污染
  });
...
describe("verifyCatalogLock（repo 实物：producer 与对账端同口径）", () => {
  it("全量对账 ok：270 entries 哈希 + 管辖面双向对账零漂移（P-v06 批次 2 增量 111→123、批次 2.6 Browser Eyes 123→124、批次 3 Backend/API/Data 124→141、批次 4 RUNTIME 物料 141→143、B6b-I FE 移植 143→168、B6b-II FE 移植 168→193、B6c BE+stacks 移植 193→228、裁定批 F/D3-R1 复核批 228→253、裁定批 G/D3-R2 收尾轮 253→270）", () => {
    const verification = verifyCatalogLock(REPO_CATALOG);
    expect(verification).toEqual({ ok: true, entries_checked: 270, drifts: [] });
  });
});
...
describe("relockCatalog（重锁计算：纯计算零写盘，Owner 裁决 2026-09-03）", () => {
  it("漂移重算：next.entries 哈希 = 落盘实际字节；refreshed 精确指路；previous 与磁盘 lock 字节零触碰", () => {
    ...
    expect(report.next.entries).toHaveLength(270);
    expect(report.previous.entries).toHaveLength(270);
  ...
  it("收敛：新增物料进 next（added + source_ref 确定性缺省）；删除物料出 next（removed）", () => {
    ...
    expect(added.next.entries).toHaveLength(271);
    expect(added.next.controlled_children.allowed).toHaveLength(271);
    expect(added.next.controlled_children.required).toHaveLength(271);
    ...
    expect(removed.next.entries).toHaveLength(270);
    expect(removed.next.controlled_children.allowed).toHaveLength(270);
    expect(removed.next.controlled_children.required).toHaveLength(270);
  });
