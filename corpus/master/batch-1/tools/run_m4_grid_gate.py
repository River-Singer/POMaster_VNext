...
职责：对 corpus/master/batch-1/ 的 truth 对象 + MASTer_master 只读源码，机械执行
...
BATCH = Path(r"D:\Vscode Documents\po-master\POMaster_VNext\corpus\master\batch-1")
...
def run_check4():
    ...
    if configured:
        ...
        scope_note = (
            "FORBIDDEN_WITHOUT_ACR 语义对象存在性检查：对 corpus/master/batch-1/"
...
def run_aggregate(gates, caps):
    ...
    scope_note = (
        ...
        "'*.json' 计数；source=corpus/master/batch-1/truth/objects/）；"
        "DENOMINATOR.TRUTH_VENDOR_COMPONENT_OBJECTS@1 value=%d（method=ls "
        "truth/objects/component/*.json 计数；source=corpus/master/batch-1/truth/"
