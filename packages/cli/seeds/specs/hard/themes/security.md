...
triggers: [security, api-input, authentication, authorization, file-processing, secrets] # 聚合注记：来源 triggers 并集；info 性注记非执行语义
...
x-aggregation: # 聚合来源（D6）：逐源 vendor pin（sha256 与卡 vendor_pin 同值）
  - seed_source: pomaster/components/frontend-hard-spec/assets/universal/04-security-protocol.md
    sha256: 4d9bb6bd34a64fcc3e362c969492bc326533e16ef4c6d2284dc01442733504e6
    seed_version: B6B-1
  - seed_source: pomaster/components/backend-hard-spec/assets/universal/10-security-protocol.md
    sha256: 56387f03edd35df50a5a701f4daab174b744d490663393f43331cf5401149f67
    seed_version: B6C
---
...
**源：FE 04 安全协议（pomaster/components/frontend-hard-spec/assets/universal/04-security-protocol.md）**
...
**源：BE 10 安全协议（pomaster/components/backend-hard-spec/assets/universal/10-security-protocol.md）**
...
**源：BE 10 安全协议 · §Contract**

安全设计必须列出资产、威胁、控制、失败语义、日志限制和验证证据。

## Checklist

**源：FE 04 安全协议 · §Checklist**
...
**源：BE 10 安全协议 · §Checklist**

- [ ] 认证、鉴权、输入、输出、文件、secret 与审计路径已检查。

## Examples

**源：FE 04 安全协议 · §Examples**
...
**源：BE 10 安全协议 · §Examples**

- 下载接口同时验证操作权限、数据范围和资源归属。

## Anti-patterns

**源：FE 04 安全协议 · §Anti-patterns**
...
**源：BE 10 安全协议 · §Anti-patterns**

- 把 token、密码或敏感 payload 写入日志与异常。

## Ownership

**源：FE 04 安全协议 · §Ownership**
...
**源：BE 10 安全协议 · §Ownership**

服务端拥有最终安全判断，安全评审者维护阻断门禁。

## Change Policy

**源：FE 04 安全协议 · §Change Policy**
