...
职责：对 migration/master-batch3/ 的 truth 对象（business-rule/state.* 455 状态对象 +
business-rule/neg.* 63 负约束对象）、MASTer_master 只读源（state-ownership-matrix /
negative-constraint / field-semantic pin 重算 + src/ 现场重扫）与
migration/master-batch2/truth/objects/page-surface/（跨批只读对账，39 蓝图载体），
...
检查项：
  ...
  03 state-machine references      状态枚举被 batch2 页面对象引用的一致性（跨批只读
    （GRN-4503）                   对账：master-batch2 page-surface 蓝图载体 39 份的
                                   states[] 引用 → master-batch3 状态枚举
...
纪律（batch3 CONVENTIONS §6 / batch2 CONVENTIONS §7 / 任务铁律）：
  - MASTer_master 绝对只读；master-batch2 只读对账（零写入）。
...
BATCH = VNEXT / "migration" / "master-batch3"
BATCH2 = VNEXT / "migration" / "master-batch2"
...
def run_check1(state_objs, matrix_rows, inv_den):
    ...
    for fname, obj in state_objs:
        ...
        if not sid or not isinstance(sid, str):
            items.append({
                ...
                "location": ("migration/master-batch3/truth/objects/business-rule/"
                             "state/" + fname),
    ...
    for sid in multi:
        items.append({
            ...
            "location": ("migration/master-batch3/truth/objects/business-rule/state/"
                         "state.*.json:" + sid),
        ...
        for b in obj["key_bindings"]["code"]:
            ...
            if not (ok and needle_ok):
                ...
                items.append({
                    ...
                    "location": ("migration/master-batch3/truth/objects/"
...
def run_check3(state_objs, carriers):
    ...
    scope_note = (
        ...
        "migration/master-batch2/truth/objects/page-surface/ 蓝图载体 %d 份——零写入；"
        "枚举=master-batch3 business-rule/state/ %d 个状态对象，STATE-* 枚举成员即状态"
