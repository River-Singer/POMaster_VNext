...
只读输入（MASTer_master 绝对只读：只 open 读，不写/不改名/不触碰 mtime）：
  - corpus/master/batch-3/inventory.yaml（M0 盘点，分母之家 + pin 事实源）
  - corpus/master/batch-3/key-binding-map.batch3.draft.yaml（BATCH-3 增量草表，登记载体）
  - corpus/master/batch-1/classification-ledger.yaml（batch1 台账：component-registry 延后注记闭环源）
  - corpus/master/batch-1/truth/objects/capability/*.json（batch1 已收编 GRID.* 3 对象，id 不冲突验证）
  - MASTer_master outputs/frontend/10_planned/ 十源（10 pin sha256 现场重算，与 inventory 逐字比对）

产出：corpus/master/batch-3/classification-ledger.yaml（10 资产逐条 §61 七分类，
...
确定性纪律（CONVENTIONS 链 batch-1 → batch-2 扩充不推翻）：
...
BATCH3_DIR = r"D:\Vscode Documents\po-master\POMaster_VNext\corpus/master/batch-3"
BATCH1_DIR = r"D:\Vscode Documents\po-master\POMaster_VNext\corpus/master/batch-1"
OUT_PATH = os.path.join(BATCH3_DIR, "classification-ledger.yaml")

INV_REL = "corpus/master/batch-3/inventory.yaml"
KBM_REL = "corpus/master/batch-3/key-binding-map.batch3.draft.yaml"
B1_LEDGER_REL = "corpus/master/batch-1/classification-ledger.yaml"
...
def build():
    entries = [
        {
            ...
            "destination": "corpus/master/batch-3/truth/objects/business-rule/",
        ...
        },
        {
            ...
            "destination": "corpus/master/batch-3/truth/objects/capability/",
        ...
        },
        {
            ...
            "batch1_remainder_closure": {
                "closes": "corpus/master/batch-1/classification-ledger.yaml entries[component-registry].rationale "
                          "注记『本批按 thread-B §2.1 只切 GRID.* 能力族 3 条……整库余量 87 条延后批次 3』",
                "grid_slice_already_ingested_batch1": {
                    "value": 3,
                    "source_ids": grid_ids,
                    "batch1_object_ids": sorted(b1_cap_ids),
                    "batch1_objects_dir": "corpus/master/batch-1/truth/objects/capability/（M1 复测三对象在册）",
                },
            ...
            "provenance": prov("project-natural", "natural", [
                ...
                src("corpus/master/batch-1/truth/objects/capability/", "GRID.* 3 对象（CAPABILITY.GRID.*）在册 M1 复测（id 不冲突验证对侧）", batch="MIG-B1"),
        ...
        },
        {
            ...
            "destination": "corpus/master/batch-3/truth/objects/field-definition/",
    ...
    data = {
        ...
        "meta": {
            "boundary_clauses": [
                {
                    "carried_from": "corpus/master/batch-1/classification-ledger.yaml meta.boundary_clauses"
            ...
            "coverage": {
                "classified": 10,
                "denominator": 10,
                "denominator_source": "corpus/master/batch-3/inventory.yaml assets[]（M0 只读盘点产出，M1 复测 len=10）",
                "ratio": "100%（10/10）",
            },
            ...
            "generated_by": "corpus/master/batch-3/tools/build_m1_classification_ledger.py",
            "idempotency": {
            ...
            "inputs": [
                ...
                "corpus/master/batch-1/classification-ledger.yaml + truth/objects/capability/（component-registry 闭环验证对侧，只读）",
            ],
            "obsolete_review": {
                "obsolete_entries": 0,
