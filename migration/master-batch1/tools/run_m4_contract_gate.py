...
对 migration/master-batch1/truth/objects/ 的 API_REQ / ERR / mock 对象执行 5 项检查，
...
def check1(objects, op_ids, path_count):
    note = (
        ...
        "（来源：migration/master-batch1/truth/objects/contract-op/ 全量 {scanned} 个现场扫描）；"
        ...
        "（suppressed_by_ledger=0）；locations 相对 migration/master-batch1/ 仓根。"
    )
    ...
    for o in applicable_objs:
        ...
        if oid not in op_ids:
            items.append(item(
                "migration/master-batch1/truth/objects/contract-op/" + o["_file"],
...
def check2(objects, endpoints):
    ...
    note = (
        ...
        "（来源：migration/master-batch1/truth/objects/contract-op/mock.*.json，源 mock-contract.yaml 13 场景→11 端点对象）；"
...
def check4(err_objs):
    ...
    for o in err_objs:
        ...
        if missing:
            items.append(item(
                "migration/master-batch1/truth/objects/error-term/" + o["_file"],
    ...
    note = (
        "分母=ERR.* 错误词条对象 {n} 个（来源：migration/master-batch1/truth/objects/error-term/，"
...
def check5(prev_runs):
    ...
    if target is None:
        run = base_run(
            ...
            "分母=authority.json boundary_rules 全部 %d 条（来源：migration/master-batch1/authority.json）；"
    ...
    if bad:
        for grn, rule in bad:
            items.append(item(
                "migration/master-batch1/gate-runs/%s/" % THEME,
    ...
    note = (
        "分母=authority.json boundary_rules 全部 {n} 条（来源：migration/master-batch1/authority.json，"
