...
（02/03 源文件与归档档案只 open 读），batch-1/2/3 truth 树只读（解析目标面），
...
fta-findings-coverage——判据面即 03_technical-assessment 的 FTA-* findings）：
  01 bp-page-linkage              BP 主文档（02_process-task-interface.yaml）抽出
    （GRN-4701）                  的页面-任务-步骤层级 vs batch-2 的 39 个
                                  ...
                                  batch-3 POLICY.STATE.* aliases 的解析账
...
纪律（任务铁律 1-4 / batch5 CONVENTIONS / batch4 §3 / batch3 §6 / batch2 硬约束 7）：
  - MASTer_master 绝对只读；产出只进 corpus/master/batch-5/gate-runs/；
...
B2_SURFACE_DIR = MIG / "master" / "batch-2" / "truth" / "objects" / "page-surface"
B3_STATE_DIR = MIG / "master" / "batch-3" / "truth" / "objects" / "business-rule" / "state"
...
def run_check1(src, models, scenes, b2_main, b2_alias, b3_alias, b3_n, chain_stats):
    ...
    for page in pages_src:
        ...
        if entry is None:
            items.append({
                ...
                "location": "corpus/master/batch-5/truth/objects/page-surface/",
                "message": "源页 %s 无对应 batch5 PAGE.MODEL 对象（层级抽取缺位）" % wf,
            })
            continue
        fname, obj = entry
        loc = "corpus/master/batch-5/truth/objects/page-surface/" + fname
        ...
        if tf_rel:
            # bp_ref.batch2_object_file 在册形态=VNEXT 仓基（"corpus/master/batch-2/…"
            # ，ingest_bp_main.py 登记口径）；与 manifest migration_pointer.object_file
            # 的 corpus/ 基（check3 用）不同源，逐字段按各自在册基解析。
            tf = VNEXT / tf_rel
            if not tf.is_file():
                items.append({
                    "rule": "bp_ref_registration_mismatch",
        ...
        for wf in sc["page_ids"]:
            if resolve_page_ref(wf, b2_main, b2_alias):
                ...
                items.append({
                    ...
                    "location": "corpus/master/batch-5/truth/objects/business-rule/"
        ...
        if entry is None:
            ...
            if not deep_equal(obj["payload"].get("scene"), sc):
                items.append({
                    ...
                    "location": "corpus/master/batch-5/truth/objects/business-rule/" + fname,
    ...
    for fname, obj in models:
        ...
        if reg_forms != rec_forms or reg.get("count") != len(rec_forms):
            ...
            items.append({
                ...
                "location": "corpus/master/batch-5/truth/objects/page-surface/" + fname,
    ...
    scope_note = (
        ...
        "batch-2 的 %d 个 page-surface 主对象（live 盘读=C5 真相面，39=%d 主对"
        ...
        "解析代码机械复算、不入本检查主分母）：02 的 %d 个 state 词形 → batch-3 "
...
def run_check2(man, inv_by_ref, pa_refs, tden_vb):
    ...
    if man_group_refs != inv_group_refs:
        items.append({
            ...
            "location": "corpus/master/batch-5/episodes/archive-manifest.yaml:documents",
    ...
    if not identity:
        items.append({
            ...
            "location": "corpus/master/batch-5/episodes/archive-manifest.yaml",
...
def run_check3(src03_raw, src03, man, inv_by_ref):
    ...
    for f in sorted(findings, key=lambda x: x["id"]):
        ...
        if sa is None:
            continue
        loc = ("corpus/master/batch-5/episodes/archive-manifest.yaml:"
