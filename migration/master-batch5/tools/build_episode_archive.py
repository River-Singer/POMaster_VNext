...
产出一个转录物到 POMaster_VNext/migration/master-batch5/episodes/：
...
yaml.safe_load 值全等验证；形态基准 migration/TOMBSTONE-RUNBOOK.md §1 +
migration/AUTHORIZATION.md 授权范围）。本批只登记（ref + 批次号 MIG-B5），不执行写，
...
OUT_DIR = r"D:\Vscode Documents\po-master\POMaster_VNext\migration\master-batch5\episodes"
...
B1_CONV = "master-batch1/CONVENTIONS.md"
B1_OBJ_DIR = "master-batch1/truth/objects/change-object"
B2_INV = "master-batch2/inventory.yaml"
B2_CONV = "master-batch2/CONVENTIONS.md"
B2_PAGE_SURFACE = "master-batch2/truth/objects/page-surface"
B5_INV = "master-batch5/inventory.yaml"
...
def collect_migration_evidence():
    ...
    m3_mentions = {}
    for b in ("master-batch1", "master-batch2", "master-batch3", "master-batch4"):
...
def build_documents(S, EV, findings):
    ...
    f = {
        ...
        "m3_1": EV["m3_mentions_of_10"].get("master-batch1"),
        "m3_2": EV["m3_mentions_of_10"].get("master-batch2"),
        "m3_3": EV["m3_mentions_of_10"].get("master-batch3"),
        "m3_4": EV["m3_mentions_of_10"].get("master-batch4"),
    ...
    specs = [
        ...
        },
        {
            ...
            "archive_reason": (
                ...
                "被消费』之说在 master-batch{{1,2,3,4}}/authority.json 全文提及次数 "
            ...
            "destination_pointers": [
                {
                    ...
                    "target": "master-batch{1,2,3,4}/authority.json",
                },
                {
                    "evidence": (
...
TOMBSTONE_FORM_REFERENCE = (
    "migration/TOMBSTONE-RUNBOOK.md §1 + migration/AUTHORIZATION.md 授权范围"
)
TOMBSTONE_COMPAT = (
    "master-batch1/batch2 tombstone 58 件同构：15 行 FROZEN 注释头逐文件 prepend、"
...
def build_tombstones(S, findings):
    ...
    for stem in ARCHIVE_STEMS:
        ...
        entries.append({
            "authorization_note": (
                "执行需 Owner 逐批写授权（precedent：migration/AUTHORIZATION.md 授权"
    ...
    skipped = [
        {
            "blindspot_metrics": {
                ...
                "census_source": (
                    "master-batch5/inventory.yaml#cross_reference_forms."
...
def build_manifest(S, EV):
    ...
    manifest = {
        ...
        "provenance_note": (
            ...
            "逐字转录（merge-preserving），不 paraphrase 不代填。约定基准：migration/"
            "master-batch1/CONVENTIONS.md → batch2 → batch3 → batch4/CONVENTIONS.md"
