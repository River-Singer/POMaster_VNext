# Evidence Spec — 死代码与重复代码

- 路径:specs/evidence/dead-code-duplicate.md
- 对象面词形:SPEC.DEAD_CODE_DUPLICATE(B2 Evidence Spec 一等对象——kind=business_rule + payload.spec_kind=evidence_spec 判别;本文件是项目可编辑要求面,对象登记时机由项目运行时 applyTransaction 决定,init 播种不写 store 对象)
- 语义锚:Project-Store PRD §13(Evidence Spec Kit)——要求面非证据面:持要求不持判定,判定值只在 Verification Result(claim 四态)/Gate Result(七态)。

## Purpose

本 Spec 定义「无新增死代码与重复」这一证据的要求:死代码(入口可达性口径)与重复块清单在座,新引入项逐条处置。
...
- 可立 claim 语句类型:「no unhandled dead or duplicated code introduced by the change」类。
...
- 本 Spec 具体条款:新引入死代码/重复为零,或逐条有处置记录。
