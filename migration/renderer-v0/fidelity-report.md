# 语义保真表 —— D25 渲染器砖 v0（legacy-outputs 投影原型）BATCH-1 十文件对照

> seq 锚：`MIG-AUTH-0001`（迁移线批次代号 MIG-AUTH；禁墙钟，本文档零时间戳，证据数字全部可由
> `tools/check_fidelity.py` + `tools/proof_byte_stable.py` 确定性重放）。
> 对照口径：渲染输出 `renders/mig-b1/outputs/frontend/10_planned/*.yaml` vs MASTer 原文件
...
| 10 | component-registry.yaml | 部分保真（收编切片） | 872 | 35 | 0 | 837 | 否 | 否（序） | 否 |
| — | 合计 | | **4075** | **3224** | **0** | **851** | | | |
...
数组源序在收编时**未入对象侧**（转录纪律「数组顺序 = 源顺序」只保住对象内逐字块的
...
2. **component-registry.yaml：837 项（上游收编切片覆盖缺口，非渲染器可逆性缺陷）**
   - 对象侧仅收编 GRID.* 切片 3/90 条（MIG-B1 转录组 D 裁定，整库其余 87 条归 BATCH-3
   ...
   - 837 = 87 条未收编条目的全部叶子，随 BATCH-3 收编后即可投影补齐。
