 ...
 * 3) 收敛：新增物料 → added 且 entries/allowed/required 三方 123→124 对账绿；
...
describe("catalog relock（漂移恢复键：漂移→relock→回绿）", () => {
  it("手改 archetype 不重锁 → status exit 1（CATALOG_LOCK_DRIFT）→ relock → status 回绿 exit 0（added=0/refreshed=1）", async () => {
    ...
    expect(relocked.result.entries_total).toBe(123);
    ...
    expect(human).toContain("catalog-lock: relocked & verified（123 entries）");
    expect(human).toContain(`~ ${TAMPER_PATH}`);

    const green = await runCatalogStatus({ catalogRoot });
    expect(green.ok).toBe(true);
    expect(green.result.lock_verification).toEqual({ ok: true, entries_checked: 123, drifts: [] });
  ...
  it("收敛：新增物料 → added 含该路径且 entries/allowed/required 三方 123→124 对账绿；删除 → removed 且对账绿", async () => {
    const catalogRoot = trackTempCatalog();
    ...
    expect(addedRun.result.entries_total).toBe(124);
    const lock = readCatalogLock(catalogRoot);
    expect(lock.entries).toHaveLength(124);
    expect(lock.controlled_children.allowed).toHaveLength(124);
    expect(lock.controlled_children.required).toHaveLength(124);
    ...
    expect(greenStatus.result.entries_total).toBe(124);
    ...
    expect(removedRun.result.entries_total).toBe(123);
    expect(readCatalogLock(catalogRoot).entries).toHaveLength(123);
    expect(verifyCatalogLock(catalogRoot).ok).toBe(true);
  });
  ...
  it("store 零依赖：无 .pomaster 的项目根同样可跑（catalog 是工具侧资产 §92.2；runCli exit 0 + §45 信封）", async () => {
    ...
    expect(envelope.result["entries_total"]).toBe(123);
