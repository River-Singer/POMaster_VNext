...
**Constitutional 档如何做到「不同流程」（L6-3）**：§90.3 的成立条件是三档走**不同**流程。tiny/normal 走 CLI 轻探针面（D-1/D-5 裁决 18，owner-adjudications.md#裁决18：原 `cli:triage` 判档入口随档位语义退役——tiny 重锚 `surface=cli:alerts`（hook 恒 exit 0 面），normal 重锚 `surface=cli:status`（未初始化 fail-closed 面），信封无 gate 执行载体）；constitutional 档走 kernel catalog v1 完整治理面（`surface=kernel:catalog+gatekeeper`，全部为 `packages/kernel` 既有 API 真实执行，禁 mock）：① `readCatalogLock` + `verifyCatalogLock`（catalog-lock 全量 sha256 对账，D24）；② profile 锚定（catalog-lock `profile=web-standard@0`——catalog 锚即档，A3 裁定 3）；③ 宪法级条目面（lock entries 的 `GATE.*` 5 条 gate recipe——A3 裁定 1 执行面=全量 + `AUTHORITY.*` 5 条 §90.2 Protected Set 之 Authority Model 锚，经共享读取器单点取得）；④ gate 判卷就绪校验（A3 裁定 1+2 执行面：5 条 gate 逐条「定义在场 + lock 哈希锚 + judging_rules 四硬判据词形（草稿升硬）」校验；诚实边界——vNext 仓库自身无业务分母，每条 gate 显式 `execution="not_run"` 计入独立披露字段 `constitutional_gate_readiness`，ok 吃「5/5 就绪 + 四规则词形完整」，不吃假判卷）+ fixture 演示判卷（kernel `normalizeGateResult` 以 catalog gate_def 锚红绿各一：绿=合式载荷归一 passed，红=`not_applicable` 缺失被 FATAL 拒收；subject `TEST.*` + `is_fixture=true` Q3 隔离，不冒充业务分母判卷）；⑤ DEF-GATEKEEPER「cannot self-approve」观测器行为校验（临时 store fixture：同 execution 既提 proposal 又 ALLOW → drift 触发；身份分离 → 不触发；空 store → 零分母显式）。三档各产出**路径签名**（执行面 / gate 集合 / artifact 集合 / profile 值），脚本机器断言两两不同——三者同路即红（§90.3「同路 = Adaptive Governance 失败」的判卷化）。

**Applicability 档（PRD v0.5.2 §15 Benchmark A，W1-A2）**：场景与 Tiny 相同（README badge 文案调整），但判卷面是 **catalog 结构化适用性**（P0.5-1）：① tiny 探针条 ok（alerts 恒 exit 0 契约——D-5 裁决 18 重锚，原 triage profile 断言退役）；② `context compile --role frontend --capability CAPABILITY.PRESENTATION` 的 catalogEntries **不含 policy.api.\*/policy.sec.\* 族**——T3 标注战役（94 条物料机器 applicability 字段）承载的更严真断言：标注前现状是 lane=frontend 全量注入 API 族（被动正确），标注后由 capabilities 轴确定性排除（主动判卷，标注回归即红）；③ 编译原始输出无 architect/research/spawn/subagent 字样（MINIMAL 档纪律延续 tiny 先例）；④ `context explain` 逐条 why：每个决策恰有一面 why（included→why_included / excluded→why_excluded），被排除的 API/Sec 族 why_excluded 携带 `capabilities=[CAPABILITY.API_CONTRACT]` 轴详情（PRD §5.4「每个决定可解释」）。**诚实边界**（研究 applicability.md R6 / Owner 裁决 8 ② O9 fixture-only）：真实 catalog 无 DB transaction / backend persistence 条目（`db_domain_entries_total=0` 如实披露，空分母不冒充判卷）——DB 排除逻辑的真判卷由 `tests/integration/catalog-applicability-case-b.spec.ts` 的 fixture DB 条目承载；本档判卷面 = capabilities 轴，change_class/profile 轴判卷由 kernel/集成 spec 承载。第四条通路的**结构可区分**：`surface="cli:context-applicability"`（context compile/explain 命令面，异于 `cli:alerts`/`cli:status` 与 `kernel:catalog+gatekeeper`）+ `excluded_refs>0` 独有判卷轴，四档路径签名两两互异由脚本机器断言（§90.3 四档延续）。
...
```bash
node benchmarks/tiny.mjs            # 单档 Tiny：断言 alerts 恒 exit 0 + 未初始化零路由段（输出无 architect/research/spawn 字样）
node benchmarks/normal.mjs          # 单档 Normal：断言 status 未初始化 fail-closed + NOT_INITIALIZED 带 hint 路标
...
- `entries[].surface` / `profile` / `matched_rule` / `evidence_grade` / `matched_keywords`：tiny/normal 探针条原文（D-5 裁决 18 重锚后 profile 位恒 null——档位词退役）；
