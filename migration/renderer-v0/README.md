# renderer-v0 —— D25 legacy-outputs 投影渲染器（v0 原型）

> seq 锚：`MIG-AUTH-0001`；批次代号 `MIG-AUTH`；禁墙钟（全部产出零时间戳）。
> 决议来源：D25（「旧 spec/outputs 目录结构以投影预设永续」）；配置骨架
> `catalog/projection-presets/legacy-outputs.yaml`；本文档是渲染器 v0 的边界声明与验收数字。

## 1. 这是什么

`tools/render_legacy_outputs.py`：从 vNext truth 对象
（`migration/master-batch*/truth/objects/**`，按 `--batch-dir` 过滤）反向组装出旧
...
|---|---|---|---|---|
| master-batch1 | 11 | **100% 字节全等** | NO_CHANGE（zero_write=true） | `4f1c681d8f0ee60f…ddb2ea` |
| master-batch2 | 9 | **100% 字节全等** | NO_CHANGE（zero_write=true） | `64a0af8dfad0d02c…fdd7cf` |
...
`catalog/projection-presets/legacy-outputs.yaml` 是映射与约束的**唯一配置源**；渲染器：
...
├── renders/
│   ├── mig-b1/                # batch1 投影产物（10 registry + render-manifest.json）
│   ├── mig-b2/                # batch2 投影产物（8 registry + render-manifest.json）
│   └── _proof/                # byte-stable 双渲染证明目录（run1/run2 独立全量产出）
└── tools/
    ├── render_legacy_outputs.py   # 渲染器本体（18 个逆组装 profile + 显式缺席登记）
...
```bash
cd POMaster_VNext/migration/renderer-v0
python tools/render_legacy_outputs.py --batch-dir ../master-batch1 --out renders/mig-b1
python tools/render_legacy_outputs.py --batch-dir ../master-batch2 --out renders/mig-b2
python tools/check_fidelity.py --rendered renders/mig-b1 --master-root "D:/Vscode Documents/MASTer_master" --out-dir .
python tools/proof_byte_stable.py --batch-dir ../master-batch1 --work renders/_proof
python tools/proof_byte_stable.py --batch-dir ../master-batch2 --work renders/_proof
```
