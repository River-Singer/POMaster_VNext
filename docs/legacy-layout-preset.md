# 遗产布局投影预设 legacy-outputs · 契约设计文档

> 状态：**契约草案（CONTRACT DRAFT）**。本批只落契约与配置骨架（`catalog/projection-presets/legacy-outputs.yaml`）；渲染器本体属后续砖（D25④），依赖 reconcile/compact 管线，本文锁定其必须满足的硬约束。
...
> 消费方：Context Projection / reporters 线（渲染器实现者）、迁移线 M5（旧编译视图冻结切换）、Owner 终审。
...
legacy-outputs 是一个 **projection preset（投影预设）**：把 Canonical State（`.pomaster/truth/objects/**` + `state/truth-index.json`）编译渲染成消费项目在旧 PoMaster 时代熟悉的目录骨架——`outputs/frontend/10_planned/…`、`30_generated/page-specs/*.md`、`handoffs/`、`bp/`——让迁移后的项目**保持视觉与工具链熟悉度**。
...
| `outputs/archive/` | 迁移时一次性归档，不持续投影 | 历史快照，非活投影 |
...
- **依赖**：reconcile/compact 管线（store 读 API、State rev、差异报告）就绪后才有稳定输入 → 自然落点在 P0.5 / 迁移线 **M5**。
...
- preset 默认 `enabled: false`：迁移项目按需启用（Minimum Sufficient Governance——未承载旧资产的新项目不背遗产骨架）。
