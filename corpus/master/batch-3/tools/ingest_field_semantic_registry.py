...
governed SEGMENT grammar:
    ...
    -> corpus/master/batch-3/truth/objects/field-definition/authenticate/<local>.json
...
def build_envelope(src, entry, index, total, source_digest, pending_count, tier1_count, tier2_count):
    ...
    escalation = (
        "regenerate via corpus/master/batch-3/tools/ingest_field_semantic_registry.py; "
        ...
        "vocab-PR seat (pending manifest: corpus/master/batch-3/"
    ...
    notes = (
        ...
        "corpus/master/batch-3/field-semantic-pending-registration.yaml；本条 id 已是 "
...
def build_manifest(src, source_digest, pending, page_segs, tier1_count, tier2_count, transcribed_count, source_count):
    return {
        "admission_gate": {
            ...
            "rule": (
                "batch-3 CONVENTIONS section 2.2 admission gate: governed SEGMENT grammar "
