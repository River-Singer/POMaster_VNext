...
- `control_data_flow` 独立映射 `CONTROL_DATA_FLOW`，不改变 `static_analysis` 的 TYPECHECK/LINT 既有义务。受信 binding 固定为 `builtin.gauntlet-lite.control-data-flow`、工具身份 `gauntlet:control-data-flow`、报告合同 `pomaster-control-data-flow-json` / `builtin.gauntlet-lite.control-data-flow/json-v1`、口径 `ui:control_flow_chain`。
...
## 36. Task telemetry 派生评估（W4-S5 · 战役 W4 R4-4 + 09-10 PRD REQ-11/§9 完整发布节）

- 需求锚（R4-4 逐字）：「reasoning/cost/Horizon：解释型规则+telemetry；预算不削弱证据/权限」+「无综合评分」+「不持久化私有思维链」。REQ-11「Reasoning/cost/Horizon 复用现有策略与派生评估，学习先形成 candidate；不自动新增治理规则」。§9「逐步收集 verified transition rate、失败方案复发、intent drift/rework、resume success、每次有效转换费用、Human intervention 与 task horizon……不虚构提升百分比；无样本/零分母记为不可计算」。W0 reuse-map REQ-11 行边界：reasoning scheduler / provider effort mapping / 预算强制 NOT FOUND 且**不在本片**——本模块是 telemetry 采集 + 派生评估呈现半边（解释型规则 = 每指标 basis 口径披露），零调度、零预算强制、零阈值、零阻断。
- 分层（plan-compiler / evidence-qualification 纯函数核先例）：`deriveTaskTelemetry(input: TaskTelemetryInput): TaskTelemetryReport` 纯函数零 IO 零 store 零墙钟（同输入重放 deep equal——A4；`inputs_fingerprint = sha256OfCanonical(整个输入)`——同指纹 = 同报告字节，快照等价物）；`gatherTaskTelemetryInput(paths, taskRef)` 是 IO 装载单一面（只读零写——readJournalLines / listExecutionRecords / countExecutionInflightReceipts / readTaskSteeringConstraints / readTaskNegativeHistory / readCheckpoint 既有装载面复用；平面损坏 fail-closed 禁静默当零记录）。输入合同 fail-closed：task_ref 词形闸（negative-history assertTaskRef 同判据）、executions[].execution_id 过 AGX 词形、负计数 SCHEMA_INVALID。
- 六指标口径终表（每指标带分母与可计算性；词形 SP 提案待追认）：① `verified_transition_rate`（§9 原文词形）——机算口径 = 锚定本任务的 claims 判定面 VERIFIED 施断/判定流转总数；生命周期转移时点的 evidence 轴不可机算（journal 是事件流无 axes 历史，reconcile §3.3 同源判词）在 basis 如实披露，转移事件计数独立呈现于 `transition_events` 字段；分母 0 = NOT_COMPUTABLE。② `rework_signal`（§9「失败方案复发/intent drift/rework」最小投影）——negative_history 条目（status 恒 REJECTED）+ claims REJECTED 计数；信号计数非评分，breakdown 双子计数。③ `steering_count`——登记约束数（declared 申报面计数，机器不验证遵守）。④ `resume_reconcile_signals`（§9「resume success」v1 可观测投影）——checkpoint 快照数 + 执行中断数；诚实披露：RECONCILE_DIRTY 阻断判卷零 journal 事件（W4-S1——阻断于一切副作用之前），不可从 journal 计数；本信号是 resume 链路载体强度计数，非恢复成败判定。⑤ `horizon`（§9「task horizon」）——journal seq 跨度（A4 禁墙钟）：EXECUTION_BEGUN seq → EXECUTION_ENDED/INTERRUPTED seq；全部封口才可算聚合跨度，open 执行 = `horizon_open` 显式（NOT_COMPUTABLE 但 horizon_rows 逐执行明细照给——聚合不吞没）；零执行 = NOT_COMPUTABLE；journal 事件残态（begun/ended 词形缺席——含部分行缺席而 horizon_open 不亮的混合残态）= NOT_COMPUTABLE 禁跨行 seq 混算冒充聚合跨度；inflight_receipts 分态 recorded|none 复用 W4-S1 既有词轴。⑥ `cost_face`（§9「每次有效转换费用」）——恒 NOT_MEASURABLE_YET：本仓无 token/费用计量源（§55.1 既有词形：缺独立信号源绝不冒充数值）；Provider 计量接入后由该信号源填充（填充位声明，非数值预填）。
- 词形纪律（SP 提案待 Owner 追认）：状态轴三值 `MEASURED | NOT_COMPUTABLE | NOT_MEASURABLE_YET`——MEASURED / NOT_MEASURABLE_YET 复用 §55.1 既有词形（production CAPABILITY_OUTCOME_METRIC_STATUS_VALUES 在册），NOT_COMPUTABLE = 信号源在座而分母为零（「零分母记为不可计算」逐字承载，新词形 SP 提案）；horizon state 两值 `closed | open`（execution 封口语义既有词形收编）；指标 key 六值轴固定序 = 报告 metrics 序（字节稳定）。全部指标禁综合评分、禁百分比虚构（v1 无跨 session 基线平面——不输出任何「较基线提升 X%」形态）。
- 红线注记常量（随报告 notes 恒在四条）：TASK_TELEMETRY_NO_COMPOSITE_SCORE_NOTE（无综合评分——六指标独立呈现禁合成单一分数或黑盒 alignment score）/ TASK_TELEMETRY_NO_CHAIN_PERSISTENCE_NOTE（不持久化私有思维链——本核输入面 = 既有治理平面结构化计数投影，无思维链输入通道）/ TASK_TELEMETRY_ADVISORY_NOTE（不设强制阈值不阻断任何流程，预算面不削弱证据与权限要求）/ TASK_TELEMETRY_NO_BASELINE_PERCENTAGE_NOTE（不做基线对比——基线建立须同类任务/Provider/预算分组且样本充分）。
- `pomaster telemetry task <task-id>`（CLI；判卷权威在 kernel，本面只做 argv 收敛、在册/kind 校验与呈现）：view review 同判词（resolveRowTargetId A5/A6 alias 收编 → OBJECT_NOT_FOUND → 非 task_object SCHEMA_INVALID）；**纯呈现零落盘**（设计裁决：telemetry 是纯派生聚合，inputs_fingerprint 即快照等价物，落盘只引入陈旧快照与真值混淆风险；REQ-11 学习面归 memory candidate 通路）——write_surface:"none" 结构级钉；ok = 报告成功产出（NOT_COMPUTABLE / NOT_MEASURABLE_YET 不是失败——诚实缺席正是交付物之一）。
- 词形 SP 提案待 Owner 追认：指标 key 六值轴 / NOT_COMPUTABLE 词形 / horizon 两值 / 命令组名 `telemetry` + 子命令 `task`（命令面非词表管辖面——P33b/P34b 先例）；复用闭集：MEASURED / NOT_MEASURABLE_YET（§55.1 在册）、closed|open（execution 封口语义）、recorded|none（W4-S1）、NOT_CONFIGURED / SCHEMA_INVALID / OBJECT_NOT_FOUND / FATAL_UNKNOWN_PREFIX / FATAL_ID_GRAMMAR。
- 对照测试：kernel task-telemetry.spec.ts 31 例（空输入零分母诚实缺席 / rate 分账与锚定过滤 / rework·resume·steering 信号计数 / horizon 全 closed·open·零执行·混合残态禁跨行混算 / census·fingerprint·词形闭包·红线注记·输入校验 / gather 全链与纯读钉）；cli telemetry-commands.spec.ts 10 例（NOT_INITIALIZED / result 结构与人读 markdown / OBJECT_NOT_FOUND·SCHEMA_INVALID·FATAL_* fail-closed / runCli 程序面 / 纯读零写字节快照与未初始化目录零落盘）。
