...
related_tools: []
legacy_id: theme:performance-and-capacity
...
triggers: [performance, capacity] # 聚合注记：来源 triggers 并集；info 性注记非执行语义
...
x-aggregation: # 聚合来源（D6）：逐源 vendor pin（sha256 与卡 vendor_pin 同值）
  - seed_source: pomaster/components/frontend-hard-spec/assets/universal/31-performance-protocol.md
    sha256: 416ff411c75e538feec3aad8be5e733b95913580fd937c0abebd16aa0d21608c
    seed_version: B6B-2
  - seed_source: pomaster/components/backend-hard-spec/assets/universal/30-performance-capacity-protocol.md
    sha256: b575dd07e4bb6def646acb55c3d62d086a9fd12035b2ab359ec353bf443773e5
    seed_version: B6C
---
...
**源：FE 31 性能协议（pomaster/components/frontend-hard-spec/assets/universal/31-performance-protocol.md）**

P1。定义首屏、包体、请求、列表、表格、图表、长任务和渲染稳定的性能规则。

**源：BE 30 性能与容量协议（pomaster/components/backend-hard-spec/assets/universal/30-performance-capacity-protocol.md）**
...
**源：BE 30 性能与容量协议 · §MUST**

- 性能结论必须绑定负载模型、数据规模、环境、版本和统计口径。

## MUST NOT

**源：FE 31 性能协议 · §MUST NOT**
...
**源：BE 30 性能与容量协议 · §SHOULD**

- 应同时观察尾延迟、错误率、资源、排队、背压和依赖瓶颈。

## Contract

**源：FE 31 性能协议 · §Contract**
...
**源：BE 30 性能与容量协议 · §Contract**

容量计划必须包含场景、预算、峰值、瓶颈、扩缩容和降级策略。

## Checklist

**源：FE 31 性能协议 · §Checklist**
...
**源：BE 30 性能与容量协议 · §Checklist**

- [ ] 连接池、线程池、队列、批量、缓存与数据库联动已验证。

## Examples

**源：FE 31 性能协议 · §Examples**

### 内容示例，可删除

筛选输入防抖并取消旧请求，详情抽屉按需加载，导出创建后台任务。

**源：BE 30 性能与容量协议 · §Examples**

- 在目标数据量和并发模型下报告 p95/p99、错误率与资源曲线。

## Anti-patterns

**源：FE 31 性能协议 · §Anti-patterns**
...
**源：BE 30 性能与容量协议 · §Anti-patterns**

- 盲目增大池大小导致下游过载和更长排队。

## Ownership

**源：FE 31 性能协议 · §Ownership**

前端性能 Owner 维护预算，模块 Owner 修复回归，后端 Owner 配合数据和接口容量。

**源：BE 30 性能与容量协议 · §Ownership**

业务 Owner 提供负载目标，Backend 与运行角色验证容量。

## Change Policy

**源：FE 31 性能协议 · §Change Policy**

预算放宽必须有数据和批准；新增重依赖或核心页面需建立基线。

**源：BE 30 性能与容量协议 · §Change Policy**
