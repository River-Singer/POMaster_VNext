...
export type BootstrapCompatibilityDisposition = "keep_temporarily" | "migrate" | "delete";

export interface BootstrapCompatibilityInventoryEntry {
  readonly id: string;
  readonly surface: string;
  readonly disposition: BootstrapCompatibilityDisposition;
...
export interface BootstrapHarnessSnapshot {
  ...
  readonly compatibility_inventory: readonly BootstrapCompatibilityInventoryEntry[];
...
export const BOOTSTRAP_COMPATIBILITY_INVENTORY: readonly BootstrapCompatibilityInventoryEntry[] = [
  {
    id: "compat.legacy-gate-detector-fallback",
    surface:
      "plan compile probes package.json (vitest/@playwright/test), browser-gate.json and .mcp.json (chrome-devtools) only when .pomaster/tools/bindings.json is absent; a present registry, including an empty one, is authoritative and an invalid registry fails closed. tools list/validate still report the absent registry explicitly.",
    disposition: "keep_temporarily",
    owner: "POMaster_VNext plan.ts/tools.ts ToolBinding integration",
    exit_condition:
      "Supported projects have migrated legacy detector inputs to validated ToolBindings, and replacement golden E2E plus plan/tool coverage is green before removing the absent-registry fallback; required tool gaps must remain REQUIRED + tool_gap.",
    coverage: [
      "packages/cli/tests/toolbinding-closed-loop.spec.ts",
      "packages/cli/tests/plan-commands.spec.ts",
      "packages/cli/tests/tools-commands.spec.ts",
    ],
  },
  {
    id: "compat.legacy-spec-directories",
    surface:
      ".pomaster/specs/hard/frontend/** and hard/backend/** coexist with canonical hard/themes, hard/stacks and evidence assets; seeds.countLegacySpecFiles reports legacy files through doctor/status without deleting or migrating project edits. spec-preplant uses canonical evidence seeds.",
    disposition: "migrate",
    owner: "POMaster_VNext seeds.ts/store-layout.ts/spec-preplant.ts spec assets",
    exit_condition:
      "Supported projects have explicitly reconciled project-owned legacy spec edits into canonical locations, doctor/status report no remaining legacy files, and replacement seed/preplant plus golden E2E coverage is green before retiring coexistence detection; no automatic deletion of user files.",
    coverage: [
      "packages/cli/tests/doctor.spec.ts",
      "packages/cli/tests/status.spec.ts",
      "packages/cli/tests/spec-preplant.spec.ts",
    ],
  },
  {
    id: "compat.legacy-objects-layout",
    surface: ".pomaster/objects/** legacy object body directory",
    disposition: "migrate",
    owner: "POMaster_VNext store-layout/init",
    exit_condition:
      "All supported projects have moved object bodies to .pomaster/truth/objects/**; init/status no longer observe legacy objects in the field.",
    coverage: ["packages/cli/tests/init.spec.ts", "packages/cli/tests/status.spec.ts"],
  },
  {
    id: "compat.legacy-thin-platform-rules",
    surface: "pre-heavy Cursor/Qoder thin rules generated by earlier init releases",
    disposition: "migrate",
    owner: "POMaster_VNext init platform adapters",
    exit_condition:
      "No supported consumer still relies on the byte-identical thin rules upgrade path; heavy generated rules are the only generated platform adapter shape.",
    coverage: ["packages/cli/tests/init.spec.ts"],
  },
  {
    id: "compat.platforms-none-minimal-entry",
    surface: "pomaster init --platforms none minimal entry",
    disposition: "keep_temporarily",
    owner: "POMaster_VNext bootstrap CLI",
    exit_condition:
      "Keep while unsupported hosts need a no-adapter bootstrap path; retire only after every supported host has a verified adapter or an explicit unsupported-host policy.",
    coverage: ["packages/cli/tests/init.spec.ts", "packages/cli/tests/doctor.spec.ts"],
  },
  {
    id: "compat.triage-profile-bootstrap-wording",
    surface: "retired triage/profile bootstrap wording and unconditional pre-work guidance",
    disposition: "delete",
    owner: "POMaster_VNext capability surfacing",
    exit_condition:
      "Deleted from generated entries and capability maps; tests must keep failing on reintroduced triage/profile bootstrap wording.",
    coverage: ["packages/cli/tests/capability-surfacing.spec.ts", "packages/cli/tests/init.spec.ts"],
  },
];
...
export async function collectBootstrapHarnessSnapshot(
  ...
  const withoutPointer: Omit<BootstrapHarnessSnapshot, "pointer"> = {
    ...
    tools,
    compatibility_inventory: BOOTSTRAP_COMPATIBILITY_INVENTORY,
...
export function renderBootstrapHarnessSummary(snapshot: BootstrapHarnessSnapshot): string[] {
  ...
  return [
    ...
    `    compatibility: ${snapshot.compatibility_inventory.length} routes inventoried（keep ${snapshot.compatibility_inventory.filter((entry) => entry.disposition === "keep_temporarily").length} / migrate ${snapshot.compatibility_inventory.filter((entry) => entry.disposition === "migrate").length} / delete ${snapshot.compatibility_inventory.filter((entry) => entry.disposition === "delete").length}）`,
