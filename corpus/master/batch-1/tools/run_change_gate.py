...
Runs the 4 task-mandated checks over corpus/master/batch-1/truth/objects/
...
def load_corpus():
    ...
    for kind_dir in sorted(p.name for p in OBJECTS_DIR.iterdir() if p.is_dir()):
        for path in sorted((OBJECTS_DIR / kind_dir).glob("*.json")):
            ...
            env["_file"] = "corpus/master/batch-1/truth/objects/%s/%s" % (
                kind_dir, path.name)
            objects[env["id"]] = env
    return objects
    ...
    for target, n in sorted(indegree.items()):
        if n > 1:
            violations.append({
                ...
                "location": "corpus/master/batch-1/truth/objects/",
