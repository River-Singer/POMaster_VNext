...
export const LAYOUT_DIRECTORIES: readonly LayoutDirSpec[] = [
  ...
  },
  {
    ...
    status: "wired",
    purpose:
      "stack overlay 播种位（18 组 × index+overlay = 36 文件——后端 14 族 B6c + 前端 4 族 B6f/B6G；installed=true 全量播种不用也在座，bound 语义以 stack.yaml 显式选型为约定载体（显式选型 = 绑定，未选型 = installed 未 bound）——B6a 只登记目录，逐批灌内容）。",
    activation_hint:
