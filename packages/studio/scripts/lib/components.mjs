...
/** 单组件族 → CSF3 story 文本（形态由 family-examples.mjs 配置表裁定）。 */
export function renderComponentStory(family) {
  ...
  if (kind === "service") {
    ...
    return [
      ...header,
      `import { ${allNames.join(", ")} } from 'ant-design-vue';`,
      ``,
      `const meta = {`,
  ...
  // 组合形态（curated/utility/default）：族导出 + 跨族组合导入，去重保序。
  const importNames = [...new Set([primary, ...secondary, ...(entry?.extraImports ?? [])])];
  ...
  if (kind === "curated" || kind === "utility") {
    ...
    const docsLine = `    docs: { description: { component: 'ant-design-vue 4.2.6 ${kindNote}${siblingNote}' } },`;
    const metaLines =
      kind === "utility"
        ? [
            `const meta = {`,
