...
export interface PermitRecord {
   ...
   * 能提供 capability/change_class/profile 等输入」；裁决 8 ②）：变更类目与治理档位。（锚：corpus/master/cutover/owner-adjudications.md#裁决8）
   * ∈ CATALOG_CHANGE_CLASS_VALUES / CATALOG_GOVERNANCE_PROFILE_VALUES（O2），签发时
   * fail-closed 词表校验；null = 未申报（缺席显式，非空串冒充）。
   * A1 裁定（2026-09-04）：governance_profile 为**信息性申报位**——不参与 checkPermit（锚：corpus/master/cutover/owner-adjudications.md#裁决11①）
...
export async function issuePermit(
  ...
  // A1 裁定（2026-09-04）：governanceProfile 是信息性申报位——词表校验保留（申报（锚：corpus/master/cutover/owner-adjudications.md#裁决11①）
